...
function lcs_setup() {
    ...
    local lcs_name=''
    gen_varname lcs_setup lcs_name
...
function lcs_run() {
    ...
    local lcs_rt_memo_map_name='' lcs_rt_memo_items_set_name=''

    gen_varname lcs_rt_memo_map lcs_rt_memo_map_name
    gen_varname lcs_rt_memo_items_set lcs_rt_memo_items_set_name
    ...
    local -n memoized_items_ref=${memo_map_ref["${lr_memo_key}"]}

    common_ref=( "${memoized_items_ref[@]}" )

    local item
    for item in "${common_ref[@]}"; do
        unset "items_set_ref[${item}]"
    done
...
declare -gri __LCS_EQ_IDX=0 __LCS_PREP_IDX=1 __LCS_KILL_IDX=2
# used during lcs_run only
declare -gri __LCS_MEMO_MAP_IDX=3 __LCS_MEMO_ITEMS_SET_IDX=4 __LCS_SEQ1_IDX=5 __LCS_SEQ2_IDX=6
