...
old_gentoo_path=${os}/${gentoo_path}/${on}
old_overlay_path=${os}/${overlay_path}/${on}
new_gentoo_path=${ns}/${gentoo_path}/${nn}
new_overlay_path=${ns}/${overlay_path}/${nn}

if [[ -e ${old_gentoo_path} ]] && [[ -e ${old_overlay_path} ]]; then
    fail "Package ${on} exists in both gentoo and overlay in old scripts"
fi

if [[ -e ${new_gentoo_path} ]] && [[ -e ${new_overlay_path} ]]; then
    fail "Package ${nn} exists in both gentoo and overlay in new scripts"
fi

if [[ ${mode} = e ]]; then
    ov=${4}
    nv=${5}
    old_gentoo_ebuild=${old_gentoo_path}/${on#*/}-${ov}.ebuild
    old_overlay_ebuild=${old_overlay_path}/${on#*/}-${ov}.ebuild
    new_gentoo_ebuild=${new_gentoo_path}/${nn#*/}-${nv}.ebuild
    new_overlay_ebuild=${new_overlay_path}/${nn#*/}-${nv}.ebuild
    ...
    if [[ -z ${old_path} ]]; then
        fail "Old version ${ov} does not exist neither in overlay nor in gentoo"
    fi

    if [[ -z ${new_path} ]]; then
        fail "New version ${nv} does not exist neither in overlay nor in gentoo"
    fi
