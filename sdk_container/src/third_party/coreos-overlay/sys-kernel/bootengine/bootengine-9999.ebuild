...
if [[ ${PV} == 9999 ]]; then
	...
	EGIT_VERSION="5d3ac4819005c92bc1b64d73ccf8d8b959443c3e" # flatcar-master
...
src_install() {
	...
	chmod +x \
		"${ED}"/usr/lib/dracut/modules.d/10*-generator/*-generator \
		"${ED}"/usr/lib/dracut/modules.d/10diskless-generator/diskless-btrfs \
		"${ED}"/usr/lib/dracut/modules.d/10networkd-dependency-generator/*-generator \
		"${ED}"/usr/lib/dracut/modules.d/03flatcar-network/parse-ip-for-networkd.sh \
		"${ED}"/usr/lib/dracut/modules.d/30disk-uuid/disk-uuid.sh \
		"${ED}"/usr/lib/dracut/modules.d/30ignition/ignition-generator \
		"${ED}"/usr/lib/dracut/modules.d/30ignition/ignition-setup.sh \
		"${ED}"/usr/lib/dracut/modules.d/30ignition/ignition-setup-pre.sh \
		"${ED}"/usr/lib/dracut/modules.d/30ignition/ignition-kargs-helper \
		"${ED}"/usr/lib/dracut/modules.d/30ignition/retry-umount.sh \
