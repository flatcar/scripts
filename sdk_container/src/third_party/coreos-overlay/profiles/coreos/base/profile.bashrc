...
vendorize_pam_files() (
    ...
    local -i mode
    case ${vpf_target_type} in
        sdk)
            # We don't care about PAM inside SDK.
            return 0
            ;;
        *-sysext-*)
            # just move files from one directory to another
            mode=MOVE
            ;;
        *)
            # files in /etc will be eventually moved to
            # /usr/share/flatcar/etc, so prepare symlinks inside
            # vendor directory to point to files inside
