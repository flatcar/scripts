...
src_configure() {
	...
	# Chrome OS specific
	mycmakeargs+=( "-DWITH_GTK=ON" )
	mycmakeargs+=( "-DBUILD_NEW_PYTHON_SUPPORT=ON" )
	mycmakeargs+=( "-DWITH_JPEG=ON" )
	mycmakeargs+=( "-DWITH_PNG=ON" )
	mycmakeargs+=( "-DWITH_TIFF=ON" )
	mycmakeargs+=( "-DWITH_V4L=ON" )

	# Setting root folder for cross-compilation
