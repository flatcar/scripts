...
EOF
  ...
  # Replace the PAM directory trees with empty directories. In theory
  # we could omit creating them, but 1. some kola tests rely on them
  # to exist and 2. empty directories are probably less confusing to
  # users then no directories at all
  sudo rm -rf "${root_fs_dir}"/etc/{security,pam.d}
  sudo mkdir -p "${root_fs_dir}"/etc/{security,pam.d}
