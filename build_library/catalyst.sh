...
generate_stage_packages_listing() {
    ...
    tar -xf "${stage}-${ARCH}-${FLAGS_version}.tar.bz2" -C "${TEMPDIR}/${stage}-listing" ./var/db/pkg
    local pkgs=() pkg slot repo
    mapfile -t pkgs -d '' < <(find "${TEMPDIR}/${stage}-listing/var/db/pkg/" -mindepth 2 -maxdepth 2 -printf '%P\0')
    for pkg in "${pkgs[@]}"; do
        slot=$(< "${TEMPDIR}/${stage}-listing/var/db/pkg/${pkg}/SLOT")
        repo=$(< "${TEMPDIR}/${stage}-listing/var/db/pkg/${pkg}/repository")
        echo "${pkg}:${slot}::${repo}"
    done >"${BUILDS}/${TYPE}-${ARCH}-${FLAGS_version}-${stage}-packages-slots-repos.txt"
