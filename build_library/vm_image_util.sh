...
set_vm_paths() {
    ...
    VM_OEM_UPD="${dst_dir}/$(_src_to_dst_name "${src_name}" "_oem-update")"
...
export_vm_oem_partition() {
    local oem_info_file="${VM_TMP_ROOT}/usr/share/oem/flatcar-oem-release"

    # placeholder file is generated by coreos-base/oem-update-vendortools
    # OEM ebuilds which support updating depend on oem-update-vendortools
    if [ ! -f "${oem_info_file}" ] ; then
        return
    fi

    local vendor_tarball="${VM_OEM_UPD}.tz"

    echo "FLATCAR_OEM_VERSION=${FLATCAR_VERSION_STRING}" | sudo tee "${oem_info_file}" >/dev/null
    echo "FLATCAR_OEM_FILE=$(basename "${vendor_tarball}")" | sudo tee -a "${oem_info_file}" >/dev/null

    info "Exporting vendor partition image to '${vendor_tarball}'"
    sudo tar -C "${VM_TMP_ROOT}/usr/share/oem/" -cJf "${vendor_tarball}" .
    VM_GENERATED_FILES+=( "${vendor_tarball}" )
}
