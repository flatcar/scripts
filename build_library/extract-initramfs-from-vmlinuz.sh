...
# check for zstdcat. Will abort the script with an error message if the tool is not present.
zstdcat -V >/dev/null
...
find_zstd_headers() {
    grep --fixed-strings --text --byte-offset --only-matching $'\x28\xB5\x2F\xFD' "$1" | cut -d: -f1
}

decompress_at() {
    # Data may not really be a valid zstd, so allow for errors.
    tail "-c+$((${2%:*} + 1))" "$1" | zstdcat 2>/dev/null || true
}
...
for OFF1 in $(find_zstd_headers "${image}")
do
    decompress_at "${image}" "${OFF1}" > "${tmp}/initrd.maybe_cpio_or_elf"
    try_extract "${tmp}/initrd.maybe_cpio_or_elf"

    for OFF2 in $(find_zstd_headers "${tmp}/initrd.maybe_cpio_or_elf")
    do
