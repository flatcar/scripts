...
do
    decompress_at "${image}" "${OFF1}" > "${tmp}/initrd.cpio.xz"

    for OFF2 in $(find_xz_headers "${tmp}/initrd.cpio.xz")
    do
        decompress_at "${tmp}/initrd.cpio.xz" "${OFF2}" > "${tmp}/initrd.cpio"
        # cpio can do strange things when given garbage, so do a basic check.
        [[ $(head -c6 "${tmp}/initrd.cpio") == 070701 ]] || continue
