...
function _sbsign_image_impl() {
    ...
    local images_local="images/${arch}-usr/${channel}-${FLATCAR_VERSION}"

    copy_from_buildcache "${images_remote}/flatcar_production_image.bin.bz2" .
    lbunzip2 --force flatcar_production_image.bin.bz2
    ...
    ./run_sdk_container -x ./ci-cleanup.sh -v "${FLATCAR_VERSION}" -U -C "${sdk_image}" \
        ./sbsign_image --board="${arch}-usr" \
                       ...
                       --output_root="/mnt/host/source/src/scripts/${images_local%%/*}" \
                       --replace --only_store_compressed
